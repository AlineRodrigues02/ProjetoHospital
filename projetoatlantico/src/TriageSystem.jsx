        ...
        {isAttendant && (
          <form onSubmit={addPatient} className="mb-6 grid grid-cols-1 gap-3 rounded-2xl bg-white p-4 shadow ring-1 ring-black/5 md:grid-cols-12">
            <div className="md:col-span-3">
              ...
              <input value={name} onChange={(e)=>setName(e.target.value)} className="mt-1 w-full rounded-lg border px-3 py-2" placeholder="Paciente" />
            </div>
            <div className="md:col-span-7">
              <label className="block text-sm font-medium text-slate-700">Motivo</label>
              <input value={reason} onChange={(e)=>setReason(e.target.value)} className="mt-1 w-full rounded-lg border px-3 py-2" placeholder="Ex.: febre, queda..." />
            </div>
            <div className="md:col-span-2 flex items-end">
              <button type="submit" className="w-full rounded-lg bg-emerald-600 px-4 py-2 font-semibold text-white">Adicionar</button>
            </div>
          </form>
        )}
